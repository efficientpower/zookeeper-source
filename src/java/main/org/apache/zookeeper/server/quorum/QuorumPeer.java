 ...
 * <li>Leader - the server will process requests and forward them to followers. A majority of followers must log the
 * request before it can be accepted.
 * </ol>
 *
 * This class will setup a datagram socket that will always respond with its view of the current leader. The response
...
public class QuorumPeer extends ZooKeeperThread implements QuorumStats.Provider {
    ...
    /*
     * ZKDatabase is a top level member of quorumpeer which will be used in all the zookeeperservers instantiated later.
    ...
    public static class QuorumServer {
        ...
        /**
         * Performs a DNS lookup of hostname and (re)creates the this.addr and this.electionAddr InetSocketAddress
         * objects as appropriate
         *
         * If the DNS lookup fails, this.addr and electionAddr remain unmodified, unless they were never set. If
         * this.addr is null, then it is set with an unresolved InetSocketAddress object. this.electionAddr is handled
         * similarly.
         */
         ...
         * @param timeout
         *            the time, in milliseconds, before {@link InetAddress#isReachable} aborts
         * @return a reachable IP address. If no such IP address can be found, just return the first IP address of the
         *         hostname.
         *
    ...
    /*
     * A peer can either be participating, which implies that it is willing to both vote in instances of consensus and
     * to elect or become a Leader, or it may be observing in which case it isn't.
     * 
     * We need this distinction to decide which ServerState to move to when conditions change (e.g. which state to
     * become after LOOKING).
     */
    ...
    /*
     * To enable observers to have no identifier, we need a generic identifier at least for QuorumCnxManager. We use the
     * following constant to as the value of such a generic identifier.
     */
    ...
    /**
     * If this is false, quorum peer server will accept another quorum peer client connection even if the authentication
     * did not succeed. This can be used while upgrading ZooKeeper server. Defaulting to false (required).
     */
    protected boolean quorumServerSaslAuthRequired;
    /**
     * If this is false, quorum peer learner will talk to quorum peer server without authentication. This can be used
     * while upgrading ZooKeeper server. Defaulting to false (required).
     */
    ...
    /**
     * Quorum learner login context name in jaas-conf file to read the kerberos security details. Defaulting to
     * 'QuorumLearner'.
     */
    protected String quorumLearnerLoginContext;

    /**
     * Quorum server login context name in jaas-conf file to read the kerberos security details. Defaulting to
     * 'QuorumServer'.
     */
    ...
    /**
     * The maximum number of threads to allow in the connectionExecutors thread pool which will be used to initiate
     * quorum server connections.
     */
    protected int quorumCnxnThreadsSize = QUORUM_CNXN_THREADS_SIZE_DEFAULT_VALUE;

    /**
     * Keeps time taken for leader election in milliseconds. Sets the value to this variable only after the completion
     * of leader election.
     */
    private long electionTimeTaken = -1;

    /**
     * @deprecated As of release 3.4.0, this class has been deprecated, since it is used with one of the udp-based
     *             versions of leader election, which we are also deprecating.
     * 
     *             This class simply responds to requests for the current leader of this node.
     *             <p>
     *             The request contains just an xid generated by the requestor.
     *             <p>
     *             The response has the xid, the id of this server, the id of the leader, and the zxid of the leader.
     *
     *
     */
    @Deprecated
        ...
        try {
            ...
            try {
                ...
                if (epochOfZxid > currentEpoch && updating.exists()) {
                    LOG.info("{} found. The server was terminated after " + "taking a snapshot but before updating current "
            ...
            if (acceptedEpoch < currentEpoch) {
                throw new IOException("The accepted epoch, " + ZxidUtils.zxidToString(acceptedEpoch) + " is less than the current epoch, "
    ...
    /**
     * This constructor is only used by the existing unit test code. It defaults to FileLogProvider persistence
     * provider.
     */
    ...
    /**
     * Check if a node is in the current view. With static membership, the result of this check will never change; only
     * when dynamic membership is introduced will this be more useful.
     */
    ...
    /**
     * Updates leader election info to avoid inconsistencies when a new server tries to join the ensemble. See
     * ZOOKEEPER-1732 for more info.
     */
