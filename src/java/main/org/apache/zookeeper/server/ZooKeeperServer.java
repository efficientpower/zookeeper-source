...
/**
 * This class implements a simple standalone ZooKeeperServer. It sets up the following chain of RequestProcessors to
 * process requests: PrepRequestProcessor -> SyncRequestProcessor -> FinalRequestProcessor
 */
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    /**
     * Creates a ZooKeeperServer instance. Nothing is setup, use the setX methods to prepare the instance (eg datadir,
     * datalogdir, ticktime, builder, etc...)
     * 
     * @throws IOException
     */
    ...
    /**
     * Creates a ZooKeeperServer instance. It sets everything up, but doesn't actually start listening for clients until
     * run() is invoked.
     * 
     * @param dataDir
     *            the directory to put the data
     */
    public ZooKeeperServer(FileTxnSnapLog txnLogFactory, int tickTime, int minSessionTimeout, int maxSessionTimeout, DataTreeBuilder treeBuilder,
            ZKDatabase zkDb) {
        serverStats = new ServerStats(this);
        ...
        LOG.info("Created server with tickTime " + tickTime + " minSessionTimeout " + getMinSessionTimeout() + " maxSessionTimeout "
                + getMaxSessionTimeout() + " datadir " + txnLogFactory.getDataDir() + " snapdir " + txnLogFactory.getSnapDir());
    }
    ...
    /**
     * This constructor is for backward compatibility with the existing unit test code. It defaults to FileLogProvider
    ...
    public void loadData() throws IOException, InterruptedException {
        /*
         * When a new leader starts executing Leader#lead, it invokes this method. The database, however, has been
         * initialized before running leader election so that the server could pick its zxid for its initial vote. It
         * does it by invoking QuorumPeer#getLastLoggedZxid. Consequently, we don't need to initialize it once more and
         * avoid the penalty of loading it a second time. Not reloading it is particularly important for applications
         * that host a large database.
         * 
         * The following if block checks whether the database has been initialized or not. Note that this method is
         * invoked by at least one other method: ZooKeeperServer#startdata.
         * 
    ...
    protected void createSessionTracker() {
        sessionTracker = new SessionTrackerImpl(this, zkDb.getSessionWithTimeOuts(), tickTime, 1, getZooKeeperServerListener());
    }
    ...
    /**
     * Sets the state of ZooKeeper server. After changing the state, it notifies the server state change to a registered
     * shutdown handler, if any.
     * <p>
     * The following are the server state transitions:
     * <li>During startup the server will be in the INITIAL state.</li>
     * <li>After successfully starting, the server sets the state to RUNNING.</li>
     * <li>The server transitions to the ERROR state if it hits an internal error. {@link ZooKeeperServerListenerImpl}
     * notifies any critical resource error events, e.g., SyncRequestProcessor not being able to write a txn to disk.
     * </li>
     * <li>During shutdown the server sets the state to SHUTDOWN, which corresponds to the server not running.</li>
     *
     * @param state
     *            new server state.
     */
    ...
    public void finishSessionInit(ServerCnxn cnxn, boolean valid) {
        ...
        try {
            ...
            if (!valid) {
                LOG.info("Invalid session 0x" + Long.toHexString(cnxn.getSessionId()) + " for client " + cnxn.getRemoteSocketAddress()
                        + ", probably expired");
                cnxn.sendBuffer(ServerCnxnFactory.closeConn);
            } else {
                LOG.info("Established session 0x" + Long.toHexString(cnxn.getSessionId()) + " with negotiated timeout " + cnxn.getSessionTimeout()
    ...
    public void processConnectRequest(ServerCnxn cnxn, ByteBuffer incomingBuffer) throws IOException {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Session establishment request from client " + cnxn.getRemoteSocketAddress() + " client's lastZxid is 0x"
        ...
        if (connReq.getLastZxidSeen() > zkDb.dataTree.lastProcessedZxid) {
            String msg = "Refusing session request for client " + cnxn.getRemoteSocketAddress() + " as it has seen zxid 0x"
                    + Long.toHexString(connReq.getLastZxidSeen()) + " our last zxid is 0x"
    ...
    /**
     * This method is used to register the ZooKeeperServerShutdownHandler to get server's error or shutdown state change
     * notifications. {@link ZooKeeperServerShutdownHandler#handle(State)} will be called for every server state changes
     * {@link #setState(State)}.
     *
     * @param zkShutdownHandler
     *            shutdown handler
     */
